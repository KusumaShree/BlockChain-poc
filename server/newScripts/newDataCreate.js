...
const create = jsonData => {
    ...
    console.log(`newDataCreate ${jsonData}`)
    console.log(`newDataCreate ${protos.compile()}`)
    ...
    .then(() => {
        ...
        const newCustodians = records.filter(record => {
            record.custodianIndex !== undefined
        })
        console.log(newCustodians)
        if(newCustodians.length > 0){
            return submitTxns(newCustodians)
        }
        return
    })
    .then(() => {
    ...
    .then(() => {
        ...
        const newReporters = records.filter(record => {
            record.reporterIndex !== undefined
        })

        console.log(newReporters)
        if(newReporters.length > 0){
            return submitTxns(newReporters)
        }
        return
    })
    .then(() => {
    ...
    .catch(err => {
        console.error(`newDataCreateError ${err.toString()}`)
        process.exit()
    })
}
